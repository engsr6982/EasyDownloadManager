...
TaskInformationDialog::TaskInformationDialog(std::shared_ptr<edm::TaskModel> model, QWidget* parent)
: QDialog(parent),
  ui(new Ui::TaskInformationDialog) {
    ...
    ui->firstTry_->setText(QString::fromStdString(time_utils::formatTime(time_utils::toClockTime(model->firstTry))));
    ui->lastTry_->setText(QString::fromStdString(time_utils::formatTime(time_utils::toClockTime(model->lastTry))));
